...
# --- Private subnets (Redshift, RDS, Lambda ENIs) ---
...
# --- Public subnet + Internet Gateway (required by NAT Gateway) ---
...
# --- NAT Gateway (allows Lambda/Glue in private subnets to reach the internet) ---
...
# --- Private route table with NAT Gateway default route ---
...
# --- VPC Endpoints (keeps S3 and SSM traffic off the NAT Gateway) ---
...
resource "aws_security_group" "lambda" {
  ...
  egress {
    description = "Allow all outbound (internet via NAT, RDS via VPC)"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
